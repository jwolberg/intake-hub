...
- Current phase: **Phase 4 — Visual Document Review: IN PROGRESS** (P4-T1..T4 done). Phase 2 complete; Phase 3 (Hardening & Polish) still open and independent.
- Current ticket: **P4-T5** (reviewer overlay UI: image + SVG boxes, two-way linking). P4-T4 (citation → bbox resolution + persistence + detail payload) is done: processing a rasterizable invoice persists resolved citations and `GET /api/invoices/:id` returns `pages` + `citations` (each with a normalized bbox). The Phase 1 live-stack exit gate is **CLEARED** (2026-05-27): Postgres round-trip passes against the live DB, `docker compose up` brings up the full stack, hub serves at `http://127.0.0.1:5173`. Remaining there: a human visual click-through of the hub UI. Commands: /docs/RUNBOOK.md.
...
- **PICK UP HERE (next session) → P4-T5 — Reviewer overlay UI (image + SVG boxes).** Phase 4 is in progress on branch `p2-track-c-reviewer-hub`: **P4-T1..T4 are DONE.** T5 renders, in the detail Source section, each page image (`GET /api/invoices/:id/pages/:n/image`) inside a positioned container with an `<svg viewBox="0 0 1 1" preserveAspectRatio="none">` overlay drawing one status-coloured `<rect>` per citation (`detail.citations`, each carrying `page_number` + a normalized `bbox`; skip citations with `bbox === null`). Two-way linking: hovering/selecting a metadata row or line item highlights the matching rect (`data-target-id` = `metadata.<field>` / `line_item.<id>.raw_description`) and clicking a rect focuses the field. Group citations by `page_number` for multi-page. Files: `/frontend/**` (`InvoiceDetail.jsx` Source section + `styles.css`). Reuse the ClinRun palette tokens (teal=extracted, amber=uncertain, etc.). Depends on P4-T4 (done). Read `docs/specs/visual-document-review.md` §4.5 + §5 and the newest `docs/implementation-notes.md` entry (P4-T4) first. **Verify in a browser** (start the hub + API per RUNBOOK; process the controlled PDF sample with `attachment_path` set).
  - Useful context already in place: `GET /api/invoices/:id` returns `pages: [{page_number,width,height}]` + `citations: [{page_number, target_id, quote, bbox|null, status, word_indices}]`; boxes are normalized `[0,1]` so the SVG needs no scaling math; `CitationStatus` ∈ extracted/uncertain/unreadable/missing (uncertain should be visually distinct + will gate clean review in T6). Processing a real PDF via the API now extracts + produces citations automatically (orchestrator `_extraction_llm` swaps in the offline PDF stand-in).
- **P4-T6** (after T5): confirm/correct/approve from the overlay, reusing the P2-C3 QC routes + P2-C4 rerun; uncertain citations gate a clean "reviewed".
